...
pystr py_str(const char* str);	pystr py_str(const char str);
...
public:
	pystr();		pystr(const char* str);		pystr(const char str);
	char* c_str();
	...
	friend pystr operator+(const char* str1, const pystr& str2);
	friend pystr operator+(const char str1, const pystr& str2);
...
pystr pystr::replace(pystr target, pystr value) {
	...
	int last = 0;
	while (1) {
		int res = str(last,"").find(target);
		if (res == -1) return ret + str(last, "");
		
		ret += str(last, last + res) + value;
		last = last + res + value_size - 1;
	}
}
...
pystr operator+(const char* str1, const pystr& str2) {
	pystr ret = py_str(str1) + str2; return ret;
}
pystr operator+(const char str1, const pystr& str2) {
	pystr ret = py_str(str1) + str2; return ret;
}

pystr py_str(const char* str) {
	pystr ret(str);
	return ret;
}
pystr py_str(const char str) {
